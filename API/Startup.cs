...
namespace API
{
    public class Startup
    {
        ...
        public void Configure(IApplicationBuilder app, IWebHostEnvironment env)
        {
            ...
            {
                // Exception page and Swagger UI enabled only in dev environment
                app.UseSwagger(); // localhost/swagger/index.html
                app.UseSwaggerUI(c => c.SwaggerEndpoint("/swagger/v1/swagger.json", "API v1"));
            }

            app.UseStatusCodePagesWithReExecute("/redirect/{0}"); // redirect non existing routes, which also appends status code
